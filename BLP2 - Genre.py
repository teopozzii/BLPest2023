...
#--------------------------------------------------------------------------
nest_dummies = pd.get_dummies(df['nest'], prefix='genre')                       # Missing line in the orignal code (Isac)
df = pd.concat([df, nest_dummies], axis=1)

#from sklearn.preprocessing import LabelEncoder  #Label Encoder is an alternative to accomunate dummies for dummy variables trap
#label_encoder = LabelEncoder()
#df['genre_encoded'] = label_encoder.fit_transform(df['nest'])
...
# Variables
    ...
    #x1 = df[['constant', 'price', 'averagescore', 'iap']].copy()
    #nest_dummies = nest_dummies.drop('genre_TOOLS', axis=1) #dummy variable trap: exclude one dummy variable
    #x1 = pd.concat([x1, nest_dummies], axis=1)  # Adding dummies for nest to x1 - characteristics (Isac)
    #x1[nest_dummies.columns] = x1[nest_dummies.columns].astype(int)
    #x1 = df[['constant', 'price', 'averagescore', 'iap', 'genre_encoded']].copy() #qualitative encoder
    #x1.columns = list(range(0, x1.shape[1]))

    x1 = df[['price', 'averagescore', 'iap']].copy()      #to avoid mutlicollinearity we do not use the constant with dummies
    x1 = pd.concat([x1, nest_dummies], axis=1)  # Adding dummies for nest to x1 - characteristics (Isac)
    x1['genre_GAME'] = x1[['genre_GAME1', 'genre_GAME2', 'genre_GAME3']].max(axis=1) #accomunate dummies to avoid multicollinearity: dummy vairables trap
    x1 = x1.drop(['genre_GAME1', 'genre_GAME2', 'genre_GAME3'], axis=1)   #we create a unique dummy GAME for the three genres GAME1, GAME2, GAME3
    x1[['genre_EDUCATION', 'genre_ENTERTAINMENT', 'genre_PERSONALIZATION', 'genre_TOOLS', 'genre_GAME']] = x1[['genre_EDUCATION', 'genre_ENTERTAINMENT', 'genre_PERSONALIZATION', 'genre_TOOLS', 'genre_GAME']].astype(int)
    #x1 = x1.drop('genre_TOOLS', axis=1)  #we drop a dummy variable to avoid collinearity: dummy variable trap

    x1.columns = list(range(0, x1.shape[1]))
    x1.loc[:, list(range(0, x1.shape[1]))] = x1.loc[:, list(range(0, x1.shape[1]))].div(scale)     #we start fom index 0 since we have no constnat | scale=100 set before. (Isac)
    ...
    #------------------------------------------------------------
    #Set of instruments - BLP1: Only Group_n as instrument (Isac)
    #------------------------------------------------------------
    IV = df.loc[:, ['group_n', 'averagescore','iap']]
    dummies_instrument = x1.columns[3:]
    IV = pd.concat([IV, x1[dummies_instrument]], axis=1)
    IV.columns = list(range(0, IV.shape[1]))
    IV.iloc[:, :3] = IV.iloc[:, :3].div(scale)
...
class BLP:
    ...
    def varcov(self,theta2): 
        ...
        if np.linalg.det(a @ np.asarray(self.invA) @ a.T)!= 0:
            ...
            inv_aAa = np.linalg.pinv(a @ np.asarray(self.invA) @ a.T)
        else:
    ...
    #Histogram for the price coefficient distribution
        ...
        alfa_i=(self.theta1[0]+alfa_i2)/100
        ...
        inelastic_df.to_csv('inelastic_data_BLP2_hp.csv', index=False)
        ...
        h.savefig('elasticities_BLP2_hp.png')
        ...
        self.theta1_results.to_csv("theta1_BLP2_hp.csv")
        self.theta2_results.to_csv("theta2_BLP2_hp.csv")

        gmmval_df = pd.DataFrame({"value of the gmm function": [self.gmmvalprint]})  #added to print GMM value in a dataframe (Isac)
        gmmval_df.to_csv('gmmValue_BLP2_hp.csv', index=False)
