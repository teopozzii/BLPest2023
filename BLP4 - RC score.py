...
# Variables
    ...
    #Allow for random coefficients on prices and avg score by interacting with demogrtaphics
    x2 = df.loc[:,['price', 'averagescore']]
    x2 = x2.div(scale)
    x2 = np.array(x2)
    x2 = np.reshape(x2, (np.shape(x2)[0],2))   #set the number of columns euqal to 2 since now we have 2 RC: price and score
...
class BLP:
    ...
    def __init__ (self,data,theta2w,mtol,niter): #initialization of the class BLP
        ...
        self.oldt2 = np.zeros((1, self.K2*(1+self.D)))  #change dimesions for oldt2 in order to be comapred with theta2 (1x4 array)
        self.gmmvalold = 0
        self.gmmdiff = 1
    ...
    #Histogram for the price coefficient distribution
        ...
        for i in range(0,self.T): 
            ...
            alfa_i2.extend(np.add(data_market@(self.theta2[1:2]).T, self.theta2[0]*v_market[:,0]))   #adjusted since we only want coefficient of theta2 relative to price
        ...
        inelastic_df.to_csv('inelastic_data_BLP4_hp.csv', index=False)
        ...
        h.savefig('elasticities_BLP4_hp.png')
        ...
        self.theta1_results.to_csv("theta1_BLP4_hp.csv")
        self.theta2_results.to_csv("theta2_BLP4_hp.csv")

        gmmval_df = pd.DataFrame({"value of the gmm function": [self.gmmvalprint]})  #added to print GMM value in a dataframe (Isac)
        gmmval_df.to_csv('gmmValue_BLP4_hp.csv', index=False)
...
theta2w = np.array([[-0.5, 0.5], [-0.5, 0.5]])                             #Adjusted for only income in the demographics
                                                               #a second row added for random coefficients on avg score
