...
# ------------------------------------------------------------
# Set of instruments - BLP3: Group_n and Hausman Price (Isac)
# ------------------------------------------------------------
df['Hausman_p'] = (
    df.groupby(['app_id', 'nest', 'year'])['price']
    .transform(lambda x: (x.sum() - x) / (x.count() - 1) if x.count() > 1 else 0)
)
...
# Variables
    ...
    #------------------------------------------------------------
    IV = df.loc[:, ['Hausman_p', 'group_n', 'constant', 'averagescore','iap']]
    IV.columns = [0,1,2,3,4]
    IV = IV.div(scale)
...
class BLP:
    ...
    #Histogram for the price coefficient distribution
        ...
        inelastic_df.to_csv('inelastic_data_BLP3_hp.csv', index=False)
        ...
        h.savefig('elasticities_BLP3_hp.png')
        ...
        self.theta1_results.to_csv("theta1_BLP3_hp.csv")
        self.theta2_results.to_csv("theta2_BLP3_hp.csv")

        gmmval_df = pd.DataFrame({"value of the gmm function": [self.gmmvalprint]})  #added to print GMM value in a dataframe (Isac)
        gmmval_df.to_csv('gmmValue_BLP3_hp.csv', index=False)

        #Hansen J test for just-identifed models
        sample_avg_g=np.mean(self.IV, axis=0) #sample average of moment condition
        adj_hansen_j_stat=len(self.gmmresid)*(sample_avg_g @ self.invA @ sample_avg_g)
        #dof=self.K1 #for an exacly identified model the number of degrees of freedom is the nummber of paramters
        dof = self.IV.shape[1] - self.K1 #for over-identified models the degrees of freedom are the number of moment conditions minus the number of parameters
        p_value = 1 - stats.chi2.cdf(adj_hansen_j_stat, dof)
        Hansen_j_df = pd.DataFrame({
            'Adjusted Hansen j stat': [adj_hansen_j_stat],
            'degrees of freedom': [dof],
            'p value' : [p_value]
        })
        Hansen_j_df.to_csv('Hansen_j_BLP3_hp.csv', index=False)
...
theta2w = np.array([-0.50, 0.50])                                                           #Adjusted for only income in the demographics
